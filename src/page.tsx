  ...
  <div className="brand-logos">
    {brands.map((brand) => (
      ...
      <section className="section services" id="services">
        ...
        <div className="service-grid">{services.map((s)=><article key={s.no}><img className="service-image" src={s.image} alt={s.title} loading="lazy"/><span className="number">{s.no}</span><h3>{s.title}</h3><p>{s.text}</p><a href={s.no === "01" ? "/services/cctv" : "#contact"}>ดูรายละเอียด <b>↗</b></a></article>)}</div>
      </section>
